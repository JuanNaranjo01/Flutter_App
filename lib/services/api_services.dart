...
class ApiService {
  ...
  /// Solicita un OTP al correo institucional del estudiante
  static Future<OtpRequestResponse> requestStudentOtp({
    required String codigoEstudiante,
    ...
    try {
      final response = await _httpClient
          .post(
            Uri.parse(
                '${ApiConfig.baseUrl}${ApiConfig.studentOtpRequestEndpoint}'),
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode({
              'codigo_estudiante': codigoEstudiante,
          ...
          .timeout(
            ApiConfig.connectionTimeout,
            onTimeout: () => throw TimeoutException(
                'Timeout solicitando OTP para el estudiante'),
          );

      final responseData = jsonDecode(response.body) as Map<String, dynamic>;
      final serverMessage =
          (responseData['message'] ?? response.body).toString();
      ...
      if (response.statusCode == 403 ||
        ...
        throw Exception(
          'El correo no pudo enviarse porque Resend está en modo de prueba. Debes verificar un dominio y usar un remitente de ese dominio para enviar OTP a otros estudiantes.',
        );
      }
  ...
  /// Verifica el OTP enviado al correo institucional del estudiante
  static Future<OtpVerifyResponse> verifyStudentOtp({
    required String codigoEstudiante,
    ...
    try {
      final response = await _httpClient
          .post(
            Uri.parse(
                '${ApiConfig.baseUrl}${ApiConfig.studentOtpVerifyEndpoint}'),
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode({
              'codigo_estudiante': codigoEstudiante,
              'otp_code': otpCode,
          ...
          .timeout(
            ApiConfig.connectionTimeout,
            onTimeout: () => throw TimeoutException(
                'Timeout verificando OTP del estudiante'),
          );
      ...
      throw Exception(responseData['message'] ?? 'OTP inválido o expirado');
    } on TimeoutException {
      rethrow;
    } on SocketException {
      rethrow;
  ...
  static Future<AttendanceResponse> registrarAsistencia({
    ...
    while (retries <= maxRetries) {
      try {
        ...
        print(
            '   - Session Token: ${sessionToken?.substring(0, 10) ?? "NULL"}...');
        ...
        print(
            '📥 Respuesta registro asistencia: status=${response.statusCode}');
  ...
  static Future<List<CourseStudent>> getCourseStudents(
    ...
    try {
      print(
          '👥 Solicitando estudiantes del curso $courseId (Endpoint Flutter)...');
      ...
      final response = await _httpClient
          .post(
            Uri.parse(
                '${ApiConfig.baseUrl}/api/flutter/curso/$courseId/estudiantes'),
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode(requestBody),
          )
          .timeout(const Duration(seconds: 10));
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> studentsJson =
            data['estudiantes'] ?? data['students'] ?? [];
        ...
        final students = studentsJson.map((json) {
  ...
  static Future<Map<String, dynamic>> getStudentAttendance(
    ...
    try {
      print(
          '📊 Solicitando detalle del estudiante $studentCode en curso $courseId (Endpoint Flutter)...');
      ...
      if (response.statusCode == 200) {
        ...
        print(
            '📋 Registros de asistencia: ${attendanceData.runtimeType} - Length: ${attendanceData is List ? attendanceData.length : "N/A"}');
  ...
  static Future<Map<String, dynamic>> getStudentAbsences(
    ...
    try {
      print(
          '📊 Solicitando faltas del estudiante $studentCode en curso $courseId (Endpoint Flutter)...');
      ...
      if (response.statusCode == 200) {
        ...
        print(
            '📋 Registros de faltas: ${absencesData.runtimeType} - Length: ${absencesData is List ? absencesData.length : "N/A"}');
  ...
  static Future<List<Periodo>> getPeriodosDisponibles() async {
    try {
      ...
      if (diagnostico.containsKey('periodos_disponibles')) {
        ...
        final periodos =
            periodosJson.map((json) => Periodo.fromJson(json)).toList();
  ...
  static Future<List<Periodo>> getPeriodos({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return periodosJson.map((json) => Periodo.fromJson(json)).toList();
      } else {
  ...
  static Future<PeriodoActual?> getPeriodoActual() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Error al obtener periodo actual: ${response.statusCode}');
      }
    } on SocketException {
      throw Exception('Error de conexión. Verifica tu conexión a internet.');
    } on TimeoutException {
  ...
  static Future<List<AsistenciaCorte>> getAsistenciasPorCorte({
    ...
    try {
      ...
      print(
          '📡 Respuesta asistencias por corte - Status: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Error al obtener asistencias por corte: ${response.statusCode}');
      }
    } on SocketException {
      throw Exception('Error de conexión. Verifica tu conexión a internet.');
    } on TimeoutException {
  ...
  static Future<List<EstadisticasCorte>> getEstadisticasPorCorte({
    ...
    try {
      ...
      final uri = Uri.parse(
              '${ApiConfig.baseUrl}/api/estadisticas_por_corte/$codigoEstudiante')
          .replace(queryParameters: queryParams.isEmpty ? null : queryParams);
      ...
      print(
          '📡 Respuesta estadísticas por corte - Status: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Error al obtener estadísticas por corte: ${response.statusCode}');
      }
    } on SocketException {
      throw Exception('Error de conexión. Verifica tu conexión a internet.');
    } on TimeoutException {
  ...
  static Future<ReporteEstudiante> getReporteEstudiante(
    ...
    try {
      print(
          '📊 Solicitando reporte detallado del estudiante $codigoEstudiante...');
      ...
      final uri = Uri.parse(
              '${ApiConfig.baseUrl}/api/reportes/estudiante/$codigoEstudiante')
          .replace(queryParameters: queryParams.isEmpty ? null : queryParams);
  ...
  static Future<ResumenEstudiante> getResumenEstudiante(
    ...
    try {
      print(
          '📊 Solicitando resumen ejecutivo del estudiante $codigoEstudiante...');

      final uri = Uri.parse(
          '${ApiConfig.baseUrl}/api/reportes/estudiante/$codigoEstudiante/resumen');
  ...
  static Future<EstudiantesEnRiesgoResponse> getEstudiantesEnRiesgo({
    ...
    try {
      ...
      final uri =
          Uri.parse('${ApiConfig.baseUrl}/api/reportes/estudiantes-en-riesgo')
              .replace(
      ...
      print(
          '📡 Respuesta estudiantes en riesgo - Status: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Error al obtener estudiantes en riesgo: ${response.statusCode}');
      }
    } on SocketException {
      throw Exception('Error de conexión. Verifica tu conexión a internet.');
    } on TimeoutException {
  ...
  static Future<ReporteDocente> getReporteDocente(
    ...
    try {
      ...
      final uri =
          Uri.parse('${ApiConfig.baseUrl}/api/reportes/docente/$correoDocente');
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Error al obtener reporte docente: ${response.statusCode}');
      }
    } on SocketException {
      throw Exception('Error de conexión. Verifica tu conexión a internet.');
    } on TimeoutException {
  ...
  static Future<Map<String, dynamic>> verifyStudentByEmail({
    ...
    try {
      print('🔍 Verificando estudiante por email: $email');
      ...
      print('❌ Error en verifyStudentByEmail: $e');
      throw Exception('Error: ${e.toString()}');
    }
  }
}
